...
class DataPartitioner(object):
    ...
    def __init__(self, data, sizes=[0.7, 0.2, 0.1], seed=1234, isNonIID=False, alpha=0, dataset=None):
        ...
        if isNonIID:
            self.partitions, self.ratio = self.__getDirichletData__(data.targets, sizes, seed, alpha)
    ...
    def __getDirichletData__(self, labels, psizes, seed, alpha):
        n_nets = len(psizes)
        K = 10
...
def partition_dataset(rank, size):
    ...
    partition = DataPartitioner(trainset, partition_sizes, isNonIID=True, alpha=0.5)
