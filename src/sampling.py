...
def cifar_unbalanced_noniid(dataset, num_users):
    """
    Sample non-I.I.D client data from CIFAR10 dataset
    :param dataset:
    :param num_users:
    ...
    Choose 5 clients that only have the data from last two classes, i.e. 9 and 10.
    
    395 shards
    
    """
    num_shards, holdout_shards, num_imgs = 500, 100, 100
    idx_shard = [i for i in range(num_shards)]
    dict_users = {i: np.array([]) for i in range(num_users)}
    idxs = np.arange(num_shards*num_imgs)
    # labels = dataset.train_labels.numpy()
    ...
    idxs_labels = np.vstack((idxs, labels))
    idxs_labels = idxs_labels[:, idxs_labels[1, :].argsort()]
    idxs = idxs_labels[0, :(num_shards-holdout_shards)*num_imgs]
    print(np.unique(idxs_labels[1, :(num_shards-holdout_shards)*num_imgs]))
    
    #rdm_choices = [0, -5, -5, 10, -10, 5, -5]
    shards_partition = [2, 2, 2] + [19 for i in range(num_users-3-1)]#[13 - random.sample(rdm_choices,1)[0] for i in range(num_users-3-1)]
    assert sum(shards_partition) < num_shards-holdout_shards
    shards_partition += [num_shards-holdout_shards-sum(shards_partition)]
    # divide and assign
    idx_shard = [i for i in range(num_shards-holdout_shards)]
    for i in range(num_users):
        rand_set = set(np.random.choice(idx_shard, shards_partition[i], replace=False))
        idx_shard = list(set(idx_shard) - rand_set)
        for rand in rand_set:
            dict_users[i] = np.concatenate(
                (dict_users[i], idxs[rand*num_imgs:(rand+1)*num_imgs]), axis=0)
    ...
    idxs = idxs_labels[0, (num_shards-holdout_shards)*num_imgs:]
    idx_shard = [i for i in range(100)]
    for i in range(3):
        rand_set = set(np.random.choice(idx_shard, 2, replace=False))
        idx_shard = list(set(idx_shard) - rand_set)
        for rand in rand_set:
    ...
    print("Number of shards for {} users: {}".format(num_users, shards_partition))
    return dict_users

def cifar_Dirichlet_noniid(labels, psizes, alpha):
    n_nets = len(psizes)
    K = 10
